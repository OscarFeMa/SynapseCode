...
class WarehouseManager:
        ...
        try:
            async with AsyncSessionLocal() as db:
                ...
                result = await db.execute(
                    select(SequentialDebateTurn).where(SequentialDebateTurn.debate_id == debate_id)
                )
    ...
    async def _update_topic_trending(self, db: AsyncSession, aggregate: DebateAggregate):
        ...
        if not trending:
            ...
            if trending.avg_duration_seconds and aggregate.duration_seconds:
                new_avg_duration = (
                    trending.avg_duration_seconds * trending.debate_count + aggregate.duration_seconds
                ) / total_debates
            ...
            if trending.avg_consensus_level is not None:
                new_avg_consensus = (
                    trending.avg_consensus_level * trending.debate_count + new_avg_consensus
                ) / total_debates
    ...
    async def _recalculate_topic_trending(self, db: AsyncSession, aggregate: DebateAggregate):
        ...
        matching = [item for item in result.scalars().all() if self._get_date_str(item.completed_at) == date_str]
        if not matching:
            return
        ...
        trending.avg_consensus_level = sum(consensus_values) / len(consensus_values) if consensus_values else None
        trending.avg_duration_seconds = sum(durations) / len(durations) if durations else None
    ...
    async def _update_consensus_patterns(self, db: AsyncSession, aggregate: DebateAggregate):
        ...
        if not pattern:
            ...
            pattern.avg_rounds_to_convergence = (
                pattern.avg_rounds_to_convergence * pattern.debate_count + aggregate.rounds_executed
            ) / total_debates
            pattern.avg_tokens_per_debate = (
                pattern.avg_tokens_per_debate * pattern.debate_count + aggregate.total_tokens_out
            ) / total_debates
    ...
    async def _update_model_performance(self, db: AsyncSession, turns: list[SequentialDebateTurn]):
        ...
        for turn in turns:
            ...
            if not perf:
                ...
                perf.avg_tokens_out = (perf.avg_tokens_out * perf.total_turns + turn.tokens_out) / total_turns
                perf.avg_latency_ms = (perf.avg_latency_ms * perf.total_turns + turn.latency_ms) / total_turns
    ...
    async def _recalculate_model_performance(self, db: AsyncSession, turns: list[SequentialDebateTurn]):
        ...
        for model_name, agent_role in affected_keys:
            ...
            perf.success_rate = sum(1 for turn in model_turns if turn.status == "completed") / total_turns
            perf.last_updated = datetime.now(UTC)
    ...
    async def _update_daily_metrics(self, db: AsyncSession, aggregate: DebateAggregate):
        ...
        # Buscar registro existente
        result = await db.execute(select(DailyMetricsSnapshot).where(DailyMetricsSnapshot.date == date_str))
        ...
        if not snapshot:
            ...
            if snapshot.avg_debate_duration_seconds and aggregate.duration_seconds:
                ...
                snapshot.avg_debate_duration_seconds = (
                    snapshot.avg_debate_duration_seconds * (total_completed - 1) + aggregate.duration_seconds
                ) / total_completed
            elif aggregate.duration_seconds:
    ...
    async def _recalculate_daily_metrics(self, db: AsyncSession, aggregate: DebateAggregate):
        ...
        date_str = self._get_date_str(aggregate.completed_at)
        result = await db.execute(select(DebateAggregate).where(DebateAggregate.completed_at.is_not(None)))
        matching = [item for item in result.scalars().all() if self._get_date_str(item.completed_at) == date_str]
        if not matching:
            return
        ...
        snapshot.avg_debate_duration_seconds = sum(durations) / len(durations) if durations else None
    ...
    async def backfill_historical_data(self) -> dict[str, int]:
        ...
        try:
            async with AsyncSessionLocal() as db:
                ...
                result = await db.execute(select(SequentialDebate).where(SequentialDebate.status == "completed"))
                ...
                result = await db.execute(select(Session).where(Session.status == "COMPLETED"))
