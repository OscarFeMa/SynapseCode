...
class WarehouseManager:
        ...
        try:
            async with AsyncSessionLocal() as db:
                ...
                if not debate or debate.status != "completed":
                    logger.warning(
                        "warehouse.sequential_debate_not_completed", debate_id=debate_id
                    )
                    return False

                # Obtener turns
                result = await db.execute(
                    select(SequentialDebateTurn).where(
                ...
                if debate.completed_at and started_at:
                    duration_seconds = int(
                        (debate.completed_at - started_at).total_seconds()
                    )
            ...
            logger.error(
                "warehouse.sequential_debate_failed", debate_id=debate_id, error=str(e)
            )
            return False
        ...
        try:
            async with AsyncSessionLocal() as db:
                ...
                if not session or session.status != "COMPLETED":
                    logger.warning(
                        "warehouse.session_not_completed", session_id=session_id
                    )
                    return False

                # Obtener agent_calls para calcular unique models
                result = await db.execute(
                    select(func.distinct(Session.agent_calls.c.model_name)).where(
                        Session.agent_calls.c.session_id == session_id
                    )
                )
                ...
                if not aggregate:
                    ...
                    aggregate.duration_seconds = duration_seconds
                    aggregate.has_tribunal_verdict = (
                        session.tribunal_verdict is not None
                    )
                    aggregate.unique_models_count = len(unique_models)
                    aggregate.updated_at = datetime.now(UTC)
            ...
            logger.error(
                "warehouse.session_failed", session_id=session_id, error=str(e)
            )
            return False

    async def _update_topic_trending(
        self, db: AsyncSession, aggregate: DebateAggregate
    ):
        ...
        if not trending:
            trending = TopicTrending(
                ...
                avg_duration_seconds=float(aggregate.duration_seconds)
            ...
            if trending.avg_duration_seconds and aggregate.duration_seconds:
                new_avg_duration = (
                    trending.avg_duration_seconds * trending.debate_count
            ...
            if trending.avg_consensus_level is not None:
                new_avg_consensus = (
                    trending.avg_consensus_level * trending.debate_count
                    + new_avg_consensus
                ) / total_debates
            ...
            trending.unique_models_count = max(
                trending.unique_models_count, aggregate.unique_models_count
            )
            trending.updated_at = datetime.now(UTC)

    async def _recalculate_topic_trending(
        self, db: AsyncSession, aggregate: DebateAggregate
    ):
        ...
        trending.unique_models_count = max(
            item.unique_models_count for item in matching
        )
        trending.updated_at = datetime.now(UTC)

    async def _update_consensus_patterns(
        self, db: AsyncSession, aggregate: DebateAggregate
    ):
        ...
        if not pattern:
            ...
            pattern.avg_rounds_to_convergence = (
                pattern.avg_rounds_to_convergence * pattern.debate_count
                + aggregate.rounds_executed
            ) / total_debates
            pattern.avg_tokens_per_debate = (
                pattern.avg_tokens_per_debate * pattern.debate_count
                + aggregate.total_tokens_out
            ) / total_debates
            success = 1.0 if aggregate.status == "completed" else 0.0
            pattern.success_rate = (
                pattern.success_rate * pattern.debate_count + success
            ) / total_debates
            pattern.updated_at = datetime.now(UTC)

    async def _update_model_performance(
        self, db: AsyncSession, turns: List[SequentialDebateTurn]
    ):
        ...
        for turn in turns:
            ...
            if not perf:
                ...
                perf.avg_tokens_out = (
                    perf.avg_tokens_out * perf.total_turns + turn.tokens_out
                ) / total_turns
                perf.avg_latency_ms = (
                    perf.avg_latency_ms * perf.total_turns + turn.latency_ms
                ) / total_turns
                success = 1.0 if turn.status == "completed" else 0.0
                perf.success_rate = (
                    perf.success_rate * perf.total_turns + success
                ) / total_turns
                perf.last_updated = datetime.now(UTC)

    async def _recalculate_model_performance(
        self, db: AsyncSession, turns: List[SequentialDebateTurn]
    ):
        ...
        for model_name, agent_role in affected_keys:
            ...
            perf.total_turns = total_turns
            perf.avg_tokens_out = (
                sum(turn.tokens_out for turn in model_turns) / total_turns
            )
            perf.avg_latency_ms = (
                sum(turn.latency_ms for turn in model_turns) / total_turns
            )
            perf.success_rate = (
                sum(1 for turn in model_turns if turn.status == "completed")
                / total_turns
            )
            perf.last_updated = datetime.now(UTC)
    ...
    async def _update_daily_metrics(self, db: AsyncSession, aggregate: DebateAggregate):
        ...
        # Buscar registro existente
        result = await db.execute(
            select(DailyMetricsSnapshot).where(DailyMetricsSnapshot.date == date_str)
        )
        ...
        if not snapshot:
            snapshot = DailyMetricsSnapshot(
                ...
                avg_debate_duration_seconds=float(aggregate.duration_seconds)
                if aggregate.duration_seconds
                else None,
            ...
            snapshot.total_debates_completed += (
                1 if aggregate.status == "completed" else 0
            )
            ...
            if snapshot.avg_debate_duration_seconds and aggregate.duration_seconds:
                ...
                snapshot.avg_debate_duration_seconds = (
                    snapshot.avg_debate_duration_seconds * (total_completed - 1)
                    + aggregate.duration_seconds
                ) / total_completed
            elif aggregate.duration_seconds:
            ...
            snapshot.active_models_count = max(
                snapshot.active_models_count, aggregate.unique_models_count
            )

    async def _recalculate_daily_metrics(
        self, db: AsyncSession, aggregate: DebateAggregate
    ):
        ...
        date_str = self._get_date_str(aggregate.completed_at)
        result = await db.execute(
            select(DebateAggregate).where(DebateAggregate.completed_at.is_not(None))
        )
        matching = [
            item
            for item in result.scalars().all()
        ...
        snapshot.total_debates_completed = sum(
            1 for item in matching if item.status == "completed"
        )
        snapshot.total_debates_failed = sum(
            1 for item in matching if item.status == "failed"
        )
        snapshot.total_turns_executed = sum(item.rounds_executed for item in matching)
        snapshot.total_tokens_generated = sum(
            item.total_tokens_out for item in matching
        )
    ...
    async def backfill_historical_data(self) -> Dict[str, int]:
        ...
        try:
            async with AsyncSessionLocal() as db:
                ...
                result = await db.execute(
                    select(SequentialDebate).where(
                        SequentialDebate.status == "completed"
                    )
                )
