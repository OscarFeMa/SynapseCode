...
from backend.database.models import AgentCall, Round, Session, SystemEvent
        ...
        try:
            for round_num in range(1, session.max_rounds + 1):
                ...
                if on_event:
                    on_event(
                        "round_start",
                        {"round_number": round_num, "total_rounds": session.max_rounds},
                    )
            ...
            calls_by_phase[phase].append(
                {
                    ...
                    if call.response and len(call.response) > 200
                    else call.response,
                }
            )
