...
from backend.database.models import ConsensusDebate
from backend.database.models import ConsensusRound as ConsensusRoundModel
...
class ConsensusDebateController:
    ...
    async def create_consensus_debate_with_id(
        ...
        logger.info(
            "consensus_debate.starting",
            session_id=session_id,
            topic=topic,
            agents_count=len(agents_config),
        )

        session = ConsensusSession(id=session_id, topic=topic, agents=agents_config)
        ...
        try:
            ...
            while (
                current_consensus < self.CONSENSUS_THRESHOLD
                and current_round < self.MAX_ROUNDS
                and len(round_3.dissent_topics) > 0
            ):
                current_round += 1
                logger.info(
                    "consensus_debate.convergence_round",
                    round=current_round,
            ...
            logger.error("consensus_debate.failed", session_id=session_id, error=str(e))
            raise
    ...
    async def _generate_agent_proposal_with_retry(
        self, session: ConsensusSession, agent: DebateAgent, max_retries: int = 2
    ) -> AgentPosition:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                last_error = e
                logger.error(
                    "consensus.proposal_attempt_failed",
                    agent=agent.name,
                    model=agent.model,
                    attempt=attempt + 1,
                    error=str(e),
        ...
        for fallback_model in fallback_models:
            ...
            try:
                ...
                if position.position and not position.position.startswith("["):
                    ...
                    logger.info(
                        "consensus.proposal_fallback_success",
                        agent=agent.name,
                        fallback_model=fallback_model,
                    )
                    return position
                ...
                logger.error(
                    "consensus.proposal_fallback_failed",
                    agent=agent.name,
                    fallback_model=fallback_model,
                    error=str(e),
                )
...
"""
    ...
    def _parse_proposal_response(self, agent: DebateAgent, response: str) -> AgentPosition:
        ...
        if "**ARGUMENTOS**:" in response:
            ...
            points = [
                p.strip()
                for p in args_section.split("\n")
                if p.strip().startswith("-")
                or p.strip().startswith("1.")
                or p.strip().startswith("2.")
                or p.strip().startswith("3.")
            ]
    ...
    async def _run_refutation_round(
        self, session: ConsensusSession, previous_round: ConsensusRoundData
    ) -> ConsensusRoundData:
        """
    ...
    async def _generate_cross_validation_with_retry(
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                logger.error(
                    "consensus.validation_attempt_failed",
                    evaluator=evaluator.name,
                    evaluated=evaluated.agent.name,
                    attempt=attempt + 1,
                    error=str(e),
                ...
                if attempt < max_retries:
                    wait_time = 2**attempt
                    logger.info("consensus.validation_retrying", wait_seconds=wait_time)
                    await asyncio.sleep(wait_time)

        # Si todo falló, retornar validación neutral con info del error
        logger.error(
            "consensus.validation_all_failed",
            evaluator=evaluator.name,
            evaluated=evaluated.agent.name,
        )
    ...
    async def _generate_cross_validation_once(
        ...
        if not evaluated.position or evaluated.position.startswith("["):
            logger.warning(
                "consensus.validation_empty_position",
                evaluator=evaluator.name,
                evaluated=evaluated.agent.name,
...
"""
        ...
        try:
            ...
            logger.error(
                "consensus.validation_once_error",
                evaluator=evaluator.name,
                evaluated=evaluated.agent.name,
                error=str(e),
            )
    ...
    def _parse_validation_response(
        self, evaluator: DebateAgent, evaluated: AgentPosition, response: str
    ) -> CrossValidation:
...
"""
    ...
    async def _generate_convergence_response(
        self, session: ConsensusSession, position: AgentPosition, round_number: int
    ) -> AgentPosition:
...
"""
    ...
    async def _call_agent(self, agent: DebateAgent, prompt: str) -> Any:
        ...
        try:
            async with AsyncSessionLocal() as db_session:
                ...
                if sync_result.get("synced"):
                    logger.info(
                        "consensus.supabase_sync_success",
                    ...
                    logger.warning(
                        "consensus.supabase_sync_failed",
                        session_id=session.id,
