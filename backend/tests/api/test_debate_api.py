...
class TestExportEndpoints:
    ...
    def test_export_json_includes_structured_metadata(self, monkeypatch):
        ...
        from backend.engine.debate_models import (
            DebateSession, DebateTurn, DebateAgent, AgentRole,
            IteracionDebate, CruzamientoCritico,
        )

        analyst = DebateAgent(
            id="a1", name="Analyst", role=AgentRole.ANALYST, node="LOCAL",
            engine="ollama", model="llama3.2:latest", provider="meta", system_prompt="",
        )
        critic = DebateAgent(
            id="a2", name="Critic", role=AgentRole.CRITIC, node="LOCAL",
        ...
        turn_1 = DebateTurn(
            turn_number=1, agent=analyst, prompt_sent="",
            response_received="La IA mejora la trazabilidad.",
            tokens_out=120, latency_ms=800, status="completed",
        )
        turn_2 = DebateTurn(
            turn_number=2, agent=critic, prompt_sent="",
            response_received="Tambien puede amplificar errores si no hay supervision.",
            tokens_out=140, latency_ms=950, status="completed",
        )

        iteration = IteracionDebate(
            iteration_number=1, phase="analysis", turns=[turn_1, turn_2],
        ...
        iteration.cruzamientos.append(
            CruzamientoCritico(
                from_agent="Critic", to_agent="Analyst",
                target_argument="La IA mejora la trazabilidad.",
                response="Eso depende de controles humanos robustos.", iteration=1,
            )
        )

        session = DebateSession(
            id="export-structured-1", topic="Impacto de la IA en auditoria",
            turns=[turn_1, turn_2], iterations=[iteration], status="completed",
            created_at=created_at, completed_at=completed_at,
            final_verdict="Adoptar IA con supervision humana.",
            structured_report={"summary": "Resumen estructurado", "consensus_level": 72},
        )
