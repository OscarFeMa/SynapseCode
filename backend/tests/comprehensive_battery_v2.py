...
# ============================================================================
print("=" * 80)
print("SYNAPSECODE COMPREHENSIVE TEST BATTERY v2")
print(f"Timestamp: {datetime.now().isoformat()}")
print("=" * 80)

# ─── 1. IMPORTS & MODULE STRUCTURE ────────────────────────────────────────────
print("\n[1/14] IMPORTS & MODULE STRUCTURE")
print("-" * 60)
...
modules_to_test = [
    ...
    ("Debug Routes", "backend.api.routes.debug", "router"),
    ("Health Routes", "backend.api.routes.health", "router"),
    ("System Routes", "backend.api.routes.system", "router"),
    ("Debate Routes", "backend.api.routes.debate", "router"),
    ("Cache Routes", "backend.api.routes.cache", "router"),
    ("WebSocket Manager", "backend.api.websocket", "WebSocketManager"),
]
...
api_tests = [
    ...
    ("GET /api/v1/system/reductio/analytics", "/api/v1/system/reductio/analytics", 200, "reductio analytics"),
    ("GET /api/v1/system/analytics", "/api/v1/system/analytics", 200, "analytics"),
    ("GET /api/v1/system/tribunal/config", "/api/v1/system/tribunal/config", 200, "tribunal config"),
    ("GET /api/v1/system/settings", "/api/v1/system/settings", 200, "settings"),
    ("GET /api/v1/system/health/sync", "/api/v1/system/health/sync", 200, "sync health"),
    ("GET /api/v1/system/worker/services", "/api/v1/system/worker/services", 200, "worker services"),
    ("GET /api/v1/system/rdp-status", "/api/v1/system/rdp-status", 200, "rdp status"),
    ...
    ("GET /debug/health-detailed", "/debug/health-detailed", 200, "debug health detailed"),
]
...
print("\n[5/14] RESPONSE SCHEMA VALIDATION")
print("-" * 60)

schema_tests = [
    ("GET /health", "/health", ["status", "timestamp", "worker_connected", "worker_ip"]),
    ...
    ("GET /debug/services", "/debug/services", ["status", "services"]),
    ("GET /debug/config", "/debug/config", ["status", "config"]),
]
...
# ─── 6. ADAPTER INSTANTIATION & METHODS ───────────────────────────────────────
...
try:
    ...
    score, details = evaluate_response("Detailed analysis with evidence", "analyst")
...
# ─── 13. WORKER SERVICE DETECTION ─────────────────────────────────────────────
print("\n[13/14] WORKER SERVICE DETECTION")
print("-" * 60)
...
try:
    ...
    for svc_name in ["ollama", "lm_studio", "jan"]:
        svc_status = status.get(svc_name, {})
...
if result.returncode == 0:
    ...
    if "passed" in output:
        ...
        if match:
            ...
            record("Pytest", "All tests pass", "passed", "output parsed")
    else:
        record("Pytest", "All tests pass", "passed", "exit code 0")
else:
    record("Pytest", "All tests pass", "failed", f"exit code={result.returncode}")

# Check test directory structure
test_dirs = ["backend/tests/unit", "backend/tests/integration", "backend/tests/api"]
for d in test_dirs:
    path = base / d
    if path.exists() and (path / "__init__.py").exists():
        test_files = list(path.glob("test_*.py"))
        record("Pytest", f"{d} structure", "passed", f"{len(test_files)} test files")
    else:
        record("Pytest", f"{d} structure", "failed", "missing")
